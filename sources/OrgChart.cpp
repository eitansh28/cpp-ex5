...
namespace ariel{
    ...
    OrgChart& OrgChart::add_root(string rootName){
        this->root.name=std::move(rootName);
        return *this;
    }
    
    OrgChart& OrgChart::add_sub(const string &commander, const string &subordinate){
        if(!findingFather(&this->root,commander, subordinate)){
            throw runtime_error("there is no such man");
        }
        return *this;
    }
    bool OrgChart::findingFather(Node* curr, const string &com, const string &sub){
        bool ans=false;
        if(curr->name==com){
            Node* temp=new Node();
            temp->name=sub;
            ...
            for(int i=0;i<curr->childern.size();++i){
                if(findingFather(curr->childern.at((unsigned int)i),com, sub)){
                    return true;
                }
            }
        }
    ...
    string& OrgChart::iterator::operator*() const {
        if(this->pointer_to_current_node==NULL){
            throw runtime_error("error!");
        }
            return this->pointer_to_current_node->name;
        // string ans="";
        // if(this->pointer_to_current_node!=NULL){
        //     this->ans=this->pointer_to_current_node->name;
        // }
    ...
    OrgChart::iterator& OrgChart::iterator::operator++() {
        if(this->flag==1){
            ...
            if(this->Queue.size()==1 && Queue.front()->childern.empty()){
            ...
            this->Stack.pop();
            for(int i=(int)curr->childern.size()-1;i>=0;--i){
    ...
    OrgChart::iterator OrgChart::begin() { 
        if(this->root.name==" "){
            throw runtime_error("enpty chart!");
        }
